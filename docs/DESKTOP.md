...
- **Screenshot** saves a full-resolution PNG of only the Android screen, without borders or backgrounds. Library → **Copy image** copies native-resolution pixels; **Save original** writes an unchanged copy to a location you choose. **Select files → Copy files** copies a real OS file list for apps that accept pasted files.
...
- **Cmd/Ctrl + Shift + R** starts/finishes recording, and **Cmd/Ctrl + Shift + S** takes a screenshot while the studio is focused.
